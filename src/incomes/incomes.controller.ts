...
export class IncomesController {
  ...
  @Post()
  create(@CurrentUser() u: AuthUser, @Body() d: CreateIncomeDto): Promise<Income> {
    return this.service.create(u.id, d);
  }

  @Get()
  list(@CurrentUser() u: AuthUser, @Query() q: IncomeQueryDto): Promise<Page<Income>> {
    return this.service.list(u.id, q);
  }

  @Get(':id')
  get(@CurrentUser() u: AuthUser, @Param('id', ParseUUIDPipe) id: string): Promise<Income> {
